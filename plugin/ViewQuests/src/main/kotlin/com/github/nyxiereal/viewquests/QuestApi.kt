...
object QuestApi {
    ...
    private fun conn(endpoint: String): HttpURLConnection =
    ...
    private fun doGet(endpoint: String): Pair<Int, String> {
        ...
        return try {
            c.requestMethod = "GET"
            c.responseCode to read(c)
        } finally { c.disconnect() }
    }

    private fun doPost(endpoint: String, body: String, extraHeaders: Map<String, String> = emptyMap()): Pair<Int, String> {
        Thread.sleep(DELAY)
        val c = conn(endpoint)
        return try {
    ...
    fun enrollQuest(questId: String): JSONObject {
        val (_, body) = doPost("/quests/$questId/enroll", JSONObject().put("platform", 0).toString())
    ...
    fun claimReward(questId: String, captchaKey: String? = null, captchaRqtoken: String? = null): Pair<Int, JSONObject> {
        ...
        if (captchaKey != null) extras["X-Captcha-Key"] = captchaKey
        if (captchaRqtoken != null) extras["X-Captcha-Rqtoken"] = captchaRqtoken
        val (code, body) = doPost("/quests/$questId/claim-reward", "{}", extras)
