...
class ThemerManagerPage : SettingsPage() {
    private val log = Logger("REthemer/Themer")
    ...
    private fun buildUI(ctx: Context) {
        ...
        if (themes.isEmpty()) {
            ...
            themes.forEach { themeName ->
                ...
                row.addView(TextView(ctx).apply {
                    ...
                    typeface = Typeface.DEFAULT_BOLD
                    setOnClickListener {
                        ThemerBridge.activateTheme(themeName)
                        toast(ctx, "Theme '$themeName' activated! Restart to see changes.")
                    }
                })
                root.addView(row)
            }
        ...
        val divider = View(ctx).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, dpT(ctx, 1)
            ).also { it.setMargins(dpT(ctx, 16), dpT(ctx, 12), dpT(ctx, 16), dpT(ctx, 12)) }
            setBackgroundColor(Color.parseColor("#2E3035"))
        }
        root.addView(divider)

        root.addView(TextView(ctx).apply {
            text = "Install themes via the Themer plugin in Aliucord settings → Themer."
            setTextColor(Color.parseColor("#72767D"))
            textSize = 11f
            gravity = Gravity.CENTER
    ...
    private fun toast(ctx: Context, msg: String) {
        ...
        t.view = TextView(ctx).apply {
            ...
            background = GradientDrawable().apply {
                setColor(Color.parseColor("#23A55A"))
